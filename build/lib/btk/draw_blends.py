...
def run_single_band(Args, blend_catalog, obs_cond, band):
    ...
    for k, entry in enumerate(blend_catalog):
        ...
        if Args.add_noise:
            ...
            noise = galsim.PoissonNoise(rng=generator, sky_level=iso_obs.mean_sky_level)
...
def generate(Args, blend_genrator, observing_generator, multiprocessing=False, cpus=1):
